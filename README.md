...
The code uses recursion to ensure that the @parameters take all of the possible combinations of values allowed by the @parameter_limits. For each instance of @parameters, we get the @named_loops updated with @modify_named_loops and then @modify_derangement updates the derangement before @output_derangement gives that to the Outputter.
